...
class HomeController extends Controller
{
    public function index()
    {
        ...
        $posts = Post::with('translations')->where('status', Post::PUBLISHED)->orderBy('created_at', 'DESC')->take(3)->get()->each(function ($item) use($locale,$fallbackLocale) {
        ...
        $featuredProducts = Product::where('featured', true)->get()->take(4);
...
//        $newProducts = $newProducts->translate($locale,$fallbackLocale);
        ...
        $saleProducts = Product::whereNotNull('sale_price')->orWhere('sale_price' ,'>',0)->get()->take(4);
