	...
	public function index() {
		...
		if ($product_info) {
			...
			foreach ($results as $result) {
			    ...
			    if (intval($result['sort_order']) < 0){
                    $data['hasInteriorImage'] = true;
                    $data['interiorImages'][] = '/image/'.$result['image'];
                }
			}


			if ($this->customer->isLogged() || !$this->config->get('config_customer_price')) {
				$data['price'] = number_format($this->currency->format($this->tax->calculate($product_info['price'], $product_info['tax_class_id'], $this->config->get('config_tax')), $this->session->data['currency']), 0, ',', ' ');
			} else {
				$data['price'] = false;
			}

			if ((float)$product_info['special']) {
				$data['special'] = number_format($this->currency->format($this->tax->calculate($product_info['special'], $product_info['tax_class_id'], $this->config->get('config_tax')), $this->session->data['currency']), 0, ',', ' ');
			} else {
				$data['special'] = false;
			}
            ...
            if($_SERVER['REMOTE_ADDR'] == '5.76.210.102'){
                $news_myvers = [];
                foreach ($news as $result_myvers_k=>$result_myvers) {
                    if($result_myvers['product_id'] == $_GET['product_id']){
                        $news_myvers[] = $result_myvers;
                    }
                }
                //мои новости начало//
                foreach ($news_myvers as $result) {
                    //var_dump($result['image']); die();
                    if ($result['image']) {
                        //$image = $this->model_tool_image->resize($result['image'], 340, 170);
                        $image = $this->model_tool_image->resize($result['image'], 1600, 500);
                        //var_dump($image); die();
                    } else {
