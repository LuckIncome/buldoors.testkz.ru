...
@section('content')
    <div class="catalog-page def-page" data-ng-controller="CatalogController as cat">
        <div class="pre-header container-fluid px-0 py-3" id="breadcrumbs">
            <div class="container">
        ...
        <div class="container" data-ng-init="cat.initFunctions('{{$category->slug}}')">
            <div class="row">
                <div class="sidebar col-3">
                    <div class="filter"><p class="title main">Фильтр</p>
                        <div class="category-select"> @if($category->slug !=='aksessuary') <select name="category"
                                                                                                   onchange="cat.refreshCategory(this.value)"> @foreach($categories as $cat)
                                    <option value="/catalog/{{$cat->slug}}"
                                            @if($category->id==$cat->id) selected @endif>{{$cat->name}}</option> @endforeach
                            </select> @endif </div>
                    ...
                    </div>{{--<a class="submit-filter">Ок</a>--}}</div>
                <div class="content col-9">
                        ...
                        <div class="col-3"
                            ...
                            <div class="item">
                                ...
                                <div class="btns"><a class="add-to-cart" data-ng-click="cat.addToCart(product)"
                                        ...
                                        href="{{route('cart.index')}}" class="add-to-cart"
                                        data-ng-if="product.stock_count && product.inCart">В корзину</a>
                                    <p class="out-of-stock" data-ng-if="!product.stock_count">На заказ</p><a
                    ...
                    <div class="text-description pb-4">
                        {!! $category->seoText !!}
                    </div>
                </div>
            </div>
        </div>@include('partials.modalFilterCatalog') </div>
@endsection
